...
"""
Credits: Code from https://github.com/mikgroup/espirit-python.
"""
...
def espirit(X, k, r, t, c):
    ...
    # local calibration using a 3x3 kernel - used to compute coil sensitivity map
    idx = 0
    ...
    # Take the Singular Value Decomposition. principle components extracted to get rid of data redundancy and clean up image from the noise
    # rank of noise is high and energy is low
    U, S, VH = np.linalg.svd(A, full_matrices=True)
    V = VH.conj().T

    # Select kernels. thresholding - selecting only principle components of column space - thresholding for energy - if energy is smaller than
    n = np.sum(S >= t * S[0])
    V = V[:, 0:n]
    ...
    # flipping is part of the convolution process lol. conjugate is found because
    ...
    # Take the iucfft - to find image of 0-padded k-space kernel
    axes = (0, 1, 2)
    kerimgs = np.zeros(np.append(np.shape(X), n)).astype(np.complex64)
    for idx in range(n):
        for jdx in range(nc):
            ker = kernels[::-1, ::-1, ::-1, jdx, idx].conj() # flipping

            kerimgs[:,:,:,jdx,idx] = fft(ker, axes) * np.sqrt(sx * sy * sz)/np.sqrt(k**p) # kernel is in image space
    print("line 75")
    #
        ...
        for jdx in range(0, sy):
            for kdx in range(0, sz):
                ...
                for ldx in range(0, nc):
                    if (s[ldx]**2 > c): #thresholding - everything above that - you have value, everything below is 0
...
def espirit_proj(x, esp):
    ...
    for qdx in range(0, esp.shape[4]): # coil-wise
        for pdx in range(0, esp.shape[3]):
            ip[:, :, :, qdx] = ip[:, :, :, qdx] + x[:, :, :, pdx] * esp[:, :, :, pdx, qdx].conj()# coil combination for each subspace

    #Finding
    for qdx in range(0, esp.shape[4]):
        for pdx in range(0, esp.shape[3]):
          proj[:, :, :, pdx] = proj[:, :, :, pdx] + ip[:, :, :, qdx] * esp[:, :, :, pdx, qdx] # proj is image in each subspace
